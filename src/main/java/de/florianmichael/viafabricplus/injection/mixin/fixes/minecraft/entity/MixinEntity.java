...
public abstract class MixinEntity implements IEntity {
    ...
    private void modifyPosWithYOffset(float offset, CallbackInfoReturnable<BlockPos> cir) {
        if (ProtocolHack.getTargetVersion().olderThanOrEquals(ProtocolVersion.v1_19_4)) {
            ...
            int j = MathHelper.floor(this.pos.y - (double) (ProtocolHack.getTargetVersion().olderThanOrEquals(ProtocolVersion.v1_18_2) && offset == 1.0E-5F ? 0.2F : offset));
    ...
    private void modifyVelocityAffectingPos(CallbackInfoReturnable<BlockPos> cir) {
        ...
        if (target.olderThanOrEquals(ProtocolVersion.v1_19_4)) {
            cir.setReturnValue(BlockPos.ofFloored(pos.x, getBoundingBox().minY - (target.olderThanOrEquals(ProtocolVersion.v1_14_4) ? 1 : 0.5000001), pos.z));
        }
    }
    ...
    private boolean allowInfiniteValues(float f) {
        return Float.isFinite(f) || ((Object) this instanceof ClientPlayerEntity && ProtocolHack.getTargetVersion().olderThanOrEquals(ProtocolVersion.v1_16_4));
    }
    ...
    private void cancelSwimming(boolean swimming, CallbackInfo ci) {
        if (ProtocolHack.getTargetVersion().olderThanOrEquals(ProtocolVersion.v1_12_2) && swimming) {
            ci.cancel();
        }
    }
